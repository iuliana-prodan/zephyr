...
void test_vec_sum_int16_op(void)
{
	...
	output = (int16_t *)malloc(VEC_LENGTH * sizeof(int16_t));
	if (!output) {
		printk("Failed to allocate memory for output result.\n");
		return;
	}
...
void test_power_int16_op(void)
{
	...
	/* Allocate output buffer */
	output = (int64_t *)malloc(sizeof(int64_t));
	if (!output) {
		printk("Failed to allocate memory for output result.\n");
		return;
	}
